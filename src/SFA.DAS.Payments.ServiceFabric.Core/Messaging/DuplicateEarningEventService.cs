...
namespace SFA.DAS.Payments.ServiceFabric.Core.Messaging
{
    public class DuplicateEarningEventService : IDuplicateEarningEventService
    {
        ...
        {
            logger.LogDebug(
                $"Checking if earning event of type {earningEvent.GetType().Name} with guid: {earningEvent.EventId} has already been received.");
            ...
            logger.LogDebug(
                $"New earnings event. Event key: {earningEventKey.LogSafeKey}, event id: {earningEvent.EventId}");
