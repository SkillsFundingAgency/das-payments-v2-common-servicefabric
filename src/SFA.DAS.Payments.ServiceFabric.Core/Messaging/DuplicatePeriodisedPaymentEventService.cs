...
namespace SFA.DAS.Payments.ServiceFabric.Core.Messaging
{
    public class DuplicatePeriodisedPaymentEventService : IDuplicatePeriodisedPaymentEventService
    {
        ...
        public DuplicatePeriodisedPaymentEventService(IPaymentLogger logger,
            IActorDataCache<PeriodisedPaymentEventKey> cache)
        {
        ...
        {
            logger.LogDebug(
                $"Checking if periodised payment event of type {periodisedPaymentEvent.GetType().Name} with guid: {periodisedPaymentEvent.EventId} has already been received.");
            ...
            {
                logger.LogWarning(
                    $"Key: {periodisedPaymentEventKey.LogSafeKey} found in the cache and is probably a duplicate.");
                return true;
            }

            logger.LogDebug(
                $"New periodised payment event. Event key: {periodisedPaymentEventKey.LogSafeKey}, event id: {periodisedPaymentEvent.EventId}");
