...
namespace SFA.DAS.Payments.ServiceFabric.Core.Messaging
{
    public class PeriodisedPaymentEventKey : EarningEventKey
    {
        ...
        {
            return
                $"{base.CreateKey()}-{TransactionType}-{Amount}-{DeliveryPeriod}-{PriceEpisodeIdentifier}-{ApprenticeshipId ?? 0}-{AccountId ?? 0}-{TransferSenderAccountId ?? 0}-{ApprenticeshipEmployerType}";
        }

        protected override string CreateLogSafeKey()
        {
            return
                $"{base.CreateLogSafeKey()}-{TransactionType}-{Amount}-{DeliveryPeriod}-{PriceEpisodeIdentifier}-{ApprenticeshipId ?? 0}-{AccountId ?? 0}-{TransferSenderAccountId ?? 0}-{ApprenticeshipEmployerType}";
        }
    }
}